...
$portfolioBalance=$portifolio['balance'];
$folioamount=$portifolio['amount'];
$message = "$$amount has been added to the portfolio $portifolioName";
//$min_amount = $portifolio['amount'];
if($amount<$folioamount){
    
    $sql = "INSERT INTO transactions(`user_id`,`folio_id`,`amount`,`type`,`status`,`date`,`unix_time`, `currency`)VALUES('$user_id','$folioId','$amount','deposit','$status','$date','$unix_time','USD')";
...
if($insertTrx){
    ...
    $update_folio=mysqli_query($conn,"UPDATE portifolios SET balance='$new_port_balance' WHERE id='$folioId'");
   ...
   // sendmail("$admin_email","$subject","$adminMessage");
      ...
      .then(()=>{
        location.replace('fund-portfolio.php')
      })
      ;
      </script>
      "; 
